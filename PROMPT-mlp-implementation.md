...
|---|---|---|
| S1 | Service core: Postgres schema, magic-link auth, book lifecycle, engine wrapper endpoints (state, forecast, trace, why_zero, events, reconcile, scenarios, compare, save/discard, edits, export), per-book locking | Integration tests for every endpoint; a concurrency test proving thread confinement (no sqlite cross-thread error under parallel requests); every money figure and diagnostic in an endpoint payload string-equal to the canonically serialized value from the direct SDK call on the same book/revision/as_of (envelope fields excluded); OpenAPI schema published |
| S2 | Agent layer: turn pipeline (interpret → guard → propose → confirm → apply → verify), Q&A read loop, transport hardening, turn/proposal persistence (incl. §2.5 staleness), cost guardrails | Ported trials T01–T12 green against the live service on `google/gemini-3.7-flash`; new invariants T13 (no unproposed/stale mutation — bypass and staleness attempts fail), T14 (question turns never write), T17 (correction scar), T18 (record-actual discriminator); cost + rate limits demonstrably enforced |
| S3 | Client foundation: Expo monorepo (RN + web), generated API client, auth + deep link, SPEC §6 screens S12 (Auth), S1 (Home/Chat), S2 (Alert variant), S4 (Proposal card), S3 (Forecast), S5 (Trace) | Playwright web E2E: auth → book (API-seeded via POST /books is acceptable in S3; the UI path is S5's gate) → mutation turn → apply proposal → forecast → trace → save; Maestro iOS-simulator smoke of the same path; zero client-side money arithmetic (lint rule + review) |
| S4 | SPEC §6 screens S6–S11 and S15: Scenarios + compare, Actuals + corrections + cutover, Plan vs actual, the three Item screens, Settings + History; server-side evaluation of the two seeded alert-rule kinds rendering on the Home banner + read-only list in Settings; R10 diagnostics render on the Actuals/F5 surface | E2E for F4 and F5 paths; T15 (`what_if` field per SPEC §2.4, present and rendered); correction flow E2E shows the scar (original visible, note mandatory); R10 diagnostics render verbatim (string-equality test against `validate()` output) |
| S5 | SPEC §6 screens S13–S14: xlsx import loop with SSE progress and reconciliation report, export, NL first-book onboarding wizard | T16 import round-trips (T06/T07 workbooks) through API and UI with reconciliation report correct; onboarding produces an accepted proposal, never a silent book; import call-cap honored |

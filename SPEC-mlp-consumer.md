...
4. **Verify** (triggering intents, enumerated: M3, M4, and any M1/M2 carrying escalation): one bounded call with the instruction, the applied intents, and `trace()` receipts; output is confirmation or corrective intents. It runs **inside the turn, on the dry-run copy, before the card is shown** rather than after the confirmation: §8 budgets accept at p95 ≤ 1 s and no model call fits inside that, and a correction that arrives before confirmation means the card the user confirms is already the right one (D-MLP-25). Corrective intents therefore replace the operations on that same card rather than opening a second one. The M1/M2 escalation class is vacuous in the MLP — the v0 grammar has no escalation slot and formula authoring is out of scope (§1) — so the two macros are the whole trigger set. Diagnostics from the SDK feed at most one repair round, as in the proto. (ADR-0030's formula-bearing class is vacuous in the MLP — formula authoring is out of scope — so macros and escalation are the verification triggers here.)
...
| `POST /turns {text, scenario?, context?}` | The chat turn; `context: "actuals_record"` marks the record-actual channel (§5-F5); returns `{kind: answer\|proposal\|clarification\|refusal, reply, receipts[], proposal?, clarification?, diagnostics[], turn_id, llm_calls, retry_after_seconds?}`. `refusal` is a §8 guardrail stopping the turn before the model — a sentence the user reads, not a status code, with one `turns` row recording it (D-MLP-24) |
...
| `POST /import` (xlsx, multipart) → `GET /imports/{id}/stream` (SSE) · `GET /imports/{id}` | §7; progress + reconciliation report. `POST /import` answers immediately with the job id and the §7.3 target, decided before the first model call; it applies nothing. The stream replays everything already emitted, so a late or reconnecting listener sees the whole run, and its final `done` event is the report plus the one proposal. `GET /imports/{id}` returns that same terminal payload as plain JSON, for a client without streaming `fetch` (added by S5, D-MLP-77) |
...
3. Target rule: on an **empty book**, import authors into base. On a **non-empty book**, import always authors into a fresh fork named from the filename — never into base — and the accept step lands there; the user promotes changes to base later through ordinary proposals. Import never merges silently and never destroys existing items. The target is decided host-side, under the book lock, before the first model call, and every authored operation is stamped with it after the guard, so the model cannot move an import somewhere else.
   **What "never into base" costs on the fork path** (found by S5 building it; D-MLP-74/75/76): three things a book-level operation would reach base through are therefore refused on a fork, and the report says so instead. The ledger is append-only and shared by every scenario, so an import into a fork authors **no events** — a one-off there is a line whose start and end are one month apart. The horizon and the opening balance are book-level, so neither is set on a fork: the book keeps its own, and a sheet row outside the horizon is reported `skipped` with the reason. And an imported line whose id collides with one the book already has is **renamed** rather than replacing it, while a change to a line the import did not author is refused — `add_item` on an existing id replaces that line, which is exactly the silent merge this rule forbids.
   **Reconciliation basis on a fork**: the fork carries the book's own plan as well as the sheet's, so a check compares what the import *added* (the fork's figures minus base's) rather than the scenario's absolute figures, and every reported row names its basis. A running-balance row cannot be compared at all on a fork, since the book keeps its own opening balance; those rows are reported `skipped` with that reason rather than given a comparison that would not hold.
4. Output: one big proposal (origin `import`) + reconciliation report. Nothing lands without the user applying the proposal.
5. Parity note for the report: 1-cent divergences on chained multiplications are expected and labeled (engine 4dp fixed-point vs Excel float; `km/notes/2026-08-22-proto-webapp-findings.md` §3). The tolerance is one flat cent, not scaled by how many figures were accumulated, and the label leaves the row **mismatched** — it never turns a divergence into a match (D-MLP-79).
6. The model says *which* cell of the sheet is a total and *what* that total means; it never supplies the figure. The sheet's side is read from the workbook by the host and the engine's side is computed by the engine, so a check cannot be satisfied by asserting (D-MLP-78).
...
- **Two further gaps found by wiring the grammar to a model (S2, D-MLP-26 and the S2 DECISIONS entry):** M1 carries one settlement term, so a split payment ("half on signing, half after 60 days") has no v0 intent form and needs either a richer slot or a host op; and no single-call verb answers "by how much do two scenarios differ at the horizon" — R9 compares the metric per period, which is what §5-F4's compare view shows, so the model quotes both closing balances instead of a difference. Both belong with the ADR-0019 scoring exercise, not with this track.

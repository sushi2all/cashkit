...
2. **Removed (content):** ADR-0020's coverage diagnostics (CK-I010…CK-I015) — implemented by S5 before this ADR reached it, then removed in the same session (D-P10-12), including the derived surfaces (`tax_coverage()`, `--coverage`, tag vocabulary) and the Italian-mechanic names that had leaked into two older codes' `suggested_fix` strings; a test now asserts no catalogue entry names a tax mechanic. `validate()` checks model consistency (PRD §10.1), not domain completeness. The mechanism apps should use instead already exists: tags, flags, and `describe_book()`.
